...
static bool GenerateGalleryImages = true;
...
ScriptResult Main()
{
  ...
  try
  {
    ...
    string nameRegex = Ini.GetValue(Path.GetFullPath(Path.Combine(Directories.CurrentModDirectory, "readme", "assets", "builds", "tools.ini")), "Maplist", "ignore");
    List<String> list = new List<String>();

    if(GenerateGalleryImages){
    ...
    GenerateData(Path.Combine(prompt.SelectedPath, "json", "serverdata.json"), gpos.Where(p => !Regex.Match(p.LayerInfo.MapName, nameRegex).Success ), mapColor);
...
IEnumerable<Gpo> GetGpos()
{
  ...
  return items.Where(x => x.IsSelected).Select(x => x.Data);
}
